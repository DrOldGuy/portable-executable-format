...
public class PEFileReader implements AutoCloseable {
  ...
  public ExportBuffers readExportBuffers() {
    ...
    Set<String> exportNames = readExportNames(headerBuffer);

    return new ExportBuffers(headerBuffer, fileName, exportNames);
  }
   ...
   * @return
   */
  private Set<String> readExportNames(ByteOrderBuffer headerBuffer) {
    int numNames = (int)headerBuffer.readUnsignedInt(EXPORT_NUM_NAMES_OFFSET);
    long namePointerTableOffset = headerBuffer.readUnsignedInt(EXPORT_NAME_POINTER_RVA_ADDRESS);
    List<Long> namePointerTable = readExportNameAddressTable(namePointerTableOffset, numNames);
    Set<String> exportNames = new TreeSet<>();
    
    for(long address : namePointerTable) {
      String name = readString(address);
  ...
  private List<Long> readExportNameAddressTable(long namePointerTableOffset, int numNames) {
    List<Long> nameAddressTable = new LinkedList<>();
